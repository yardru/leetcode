...
public:
    ...
    bool solveSudokuMainLoop(std::vector<std::vector<char>> & board) {
        while (n > 0) {
            if (heap[0]->dn < 1)
                return false;
            if (heap[0]->dn > 1) {
...
private:
    ...
    void put_digit(int d) {
        ...
        std::swap(heap[0]->pos_in_heap, heap[n]->pos_in_heap);
        heapify_down(0);
    ...
    void heapify_up(int i) {
        ...
        std::swap(heap[p]->pos_in_heap, heap[i]->pos_in_heap);
        heapify_up(p);
    }
    void heapify_down(int i) {
        ...
        std::swap(heap[next]->pos_in_heap, heap[i]->pos_in_heap);
        heapify_down(next);
    }
};
